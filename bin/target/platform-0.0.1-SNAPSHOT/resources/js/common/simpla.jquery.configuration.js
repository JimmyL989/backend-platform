jQuery(document).ready(function(){
	...
	//Sidebar Accordion Menu:
		...
		jQuery("#isshow").show();
		jQuery("#main-nav li a.current").parent().find("ul").slideToggle(1000); // Slide down the current menu item's sub menu
		
		jQuery("#main-nav li a.nav-top-item").click( // When a top menu item is clicked...
			function () {
				jQuery(this).parent().siblings().find("ul").slideUp(500); // Slide up all sub menus except the one clicked
				jQuery(this).next().slideToggle(500); // Slide down the clicked sub menu
