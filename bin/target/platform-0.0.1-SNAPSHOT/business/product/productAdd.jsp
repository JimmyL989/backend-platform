...
<script type="text/javascript">
	 ...
	 $(function() {
		...
		$('#form').form({
			...
			onSubmit : function() {
				...
				if ($('#state').val() == '待审批' || $('#state').val() == '审批通过' || $('#state').val() == '已发送') {
					parent.$.messager.alert('错误', '该状态无法修改', 'error');
					return false;
				}
				
				if(cuserid != '-1' && '${sessionScope.logon_sysuser.userid}' != cuserid && '${sessionScope.logon_sysuser.userid}' != '1'){
					parent.$.messager.alert('错误', '此短信非本人制作，无权修改。', 'error');
					return false;
				}
	...
	function getProduct5day(regionId) {
		...
		var tommow_milliseconds=myDate.getTime()+1000*60*60*24;       
    	var tommow = new Date();       
        tommow.setTime(tommow_milliseconds); 
		
		$.post('${pageContext.request.contextPath}/business/controller/ProductController/queryMessageBody.do', {
						productId : productId, regionId : regionId, minTime : myDate.format("yyyy-MM-dd"), maxTime : tommow.format("yyyy-MM-dd")
					}, function(result) {
						if (result.success) {
							//parent.$.messager.alert('提示', result.msg, 'info');
							$('#messageBody').val(result.obj.content);
							$("#form input[name=instanceid]").val(result.obj.instanceid);
							validMaxSize();
							...
							cuserid = result.obj.cuserid;
							
							if (result.obj.sendstate == '0' || result.obj.sendstate == '-1' || result.obj.sendstate == '1'){
								$('#messageBody').attr('readonly', 'readonly');
							} else {
								// 权限验证
								if('${sessionScope.logon_sysuser.userid}' != result.obj.cuserid && '${sessionScope.logon_sysuser.userid}' != '1'){
									$('#messageBody').attr('readonly', 'readonly');
									// 消息将显示在顶部中间
									$.messager.show({
										title:'我的消息',
							...
							$('#messageBody').removeAttr('readonly');
							$('#messageBody').val('');
							$("#form input[name=instanceid]").val('');
						}
						parent.$.messager.progress('close');
					}, 'JSON');
			...
			onLoadSuccess : function() {
				...
				if($('#state').val() == '待录入') {
					...
					for(var i = 0; i < $('#forecast').datagrid('getRows').length; i++) {
						if($('#forecast').datagrid('getRows')[i].time == '48') {
							r += '[农事气象-' + $('#cc1').combobox('getText') + ']' + $('#cc').combobox('getText') + '：' + $('#forecast').datagrid('getRows')[i].content;
						}else if($('#forecast').datagrid('getRows')[i].time == '72') {
							r += $('#forecast').datagrid('getRows')[i]._forecast_time + '，' + $('#forecast').datagrid('getRows')[i].weather;
						}else if($('#forecast').datagrid('getRows')[i].time == '108') {
							r += '，' + $('#forecast').datagrid('getRows')[i]._forecast_time + '，' + $('#forecast').datagrid('getRows')[i].weather + '。';
						}
					}
					r = r.replace("℃","度");
		...
		<div data-options="region:'north',title:'',border:false" style="height: 60px; overflow: hidden;">
			<table class="table table-hover table-condensed" style="height: 60px">
				<tr>
					<th width="70px">产品</th>
					<td width="100px">
					<input id="cc1" name="cc1" class="easyui-combobox" data-options="editable:false,required:true, valueField: 'Productid',textField: 'Product_name', 
						...
						$('#cc1').combobox({
							onLoadSuccess : function() {
								if('<%=request.getParameter("productid")%>' != 'null')
									$('#cc1').combobox('select', '<%=request.getParameter("productid")%>');
							}
						});
					</script>
					</td>
					...
					<select id="cc" name="cc" class="easyui-combotree" style="width:200px;" required="required" data-options="onSelect: function(rec){  
        				var tree = $(this).tree;  
       					var isLeaf = tree('isLeaf', rec.target);  
        				if (!isLeaf) {  
           					$('#cc').combotree('clear');  
        				}  else {
        					getProduct5day(rec.id);
					...
					<script>
						$('#cc').combotree({
							onLoadSuccess : function(param) {
								if('<%=request.getParameter("region_code")%>' != 'null')
									$('#cc').combotree('setValue', '<%=request.getParameter("region_code")%>');
           			...
           			<td width="120px" style="padding-top: 12px"><input id="state" name="state"  type="text" style="height: 16px; width: 120px;line-height: normal;" readonly="readonly" value=""></input>
           			</td>
				</tr>
			</table>
		</div>
			...
			<div class="easyui-layout" data-options="fit:true,border:false" >
				...
				<div data-options="region:'west',border:false" title="" style="overflow: hidden;width: 380px">
					<table class="table table-hover table-condensed">
						...
						<tr></tr>
						<tr>
							<th width="20px">短信内容</th> 
							<td>
							<!-- 350   20-320    -->
							<textarea name="messageBody" placeholder="请编辑短信内容..." id="messageBody" rows="13" cols="" style="width: 320px;resize: none;font-size:16px; color:#000; " onPropertyChange="validMaxSize()" oninput="validMaxSize()" class="easyui-validatebox" data-options="required:true,validType:'maxCharLength[280]'">${resource.memo}</textarea>
							</td>
							</tr>
							<tr>
