...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" title="" style="overflow: hidden;">
		 ...
		 <input name="crops_period_id" type="hidden" value="${cp.crops_period_id}"/>
			<table class="table table-hover table-condensed">
				<tr>
					<th width="80px">作物</th>
					<td width="80px">
					<input id="crops_id" name="crops_id" readonly="readonly" class="easyui-combobox" data-options="required:true,width:320, valueField: 'crops_id',textField: 'crops_name', 
								url: '${pageContext.request.contextPath}/business/controller/CropController/queryCropCombo.do',onLoadSuccess : function() {
								$('#crops_id').combobox('select', '${cp.crops_id}');
							}" />	
					</td>
				</tr>
				<tr>
					<th>生育时期</th>
					<td colspan="3"><input id="period_id" name="period_id" class="easyui-combobox" readonly="readonly" data-options="required:true,width:400, valueField: 'period_id',textField: 'period_name', 
								url: '${pageContext.request.contextPath}/business/controller/PeriodController/queryPeriodCombo.do',onLoadSuccess : function() {
								$('#period_id').combobox('select', '${cp.period_id}');
							}" />   </td>				
				</tr>
				<tr>
					<th>范 围</th>
					<td colspan="3">
					<input class="easyui-validatebox" style="width: 300 px" id="sdate" name="sdate" placeholder="点击选择时间" onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'edate\')}'})" readonly="readonly" value="${cp.sdate }"/>
					至
					<input style="width: 300 px" class="easyui-validatebox" id="edate" name="edate" placeholder="点击选择时间" onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'sdate\')}'})" readonly="readonly" value="${cp.edate }"/>   </td>
				</tr>
			</table>
		</form> 
	</div>
