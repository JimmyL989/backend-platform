...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/business/controller/AdviceController/queryAnalysis.do',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					var str = '';
						str += $.formatString('<img onclick="editFun(\'{0}\');" src="{1}" title="编辑"/>', row.advice_analysis_id, '${pageContext.request.contextPath}/resources/image/edit.gif');
					str += '&nbsp;';
						str += $.formatString('<img onclick="deleteFun(\'{0}\');" src="{1}" title="删除"/>', row.advice_analysis_id, '${pageContext.request.contextPath}/resources/image/feizhi.gif');
					return str;
				}
			} ] ],
			toolbar : '#toolbar',
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前农作物及其关联关系？', function(b) {
			if (b) {
					...
					$.post('${pageContext.request.contextPath}/business/controller/AdviceController/deleteAnalysis.do', {
						id : id
					}, function(result) {
						if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 600,
			height : 500,
			href : '${pageContext.request.contextPath}/business/controller/AdviceController/editAnalysisPage.do?id=' + id,
			buttons : [ {
				text : '保存',
				handler : function() {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 600,
			height : 500,
			href : '${pageContext.request.contextPath}/business/advice/analysis/analysisAdd.jsp',
			buttons : [ {
				text : '保存',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
